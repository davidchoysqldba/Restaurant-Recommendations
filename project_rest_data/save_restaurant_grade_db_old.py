...
class FindRestaurantData(luigi.Task):
    ...
    def requires(self):
        ...
        if dbobj:
            resultobj = dbobj.executeQuery("select distinct file_name from restaurant_inbox")
            #print(resultobj)
    ...
    def output(self):
        return luigi.LocalTarget(self.dt.strftime('data/find_rest_data_%Y_%m_%d.log'))
...
class SaveRestaurantData(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(self.dt.strftime('data/save_rest_data_' + self.filename[-31:] + '_%Y_%m_%d.log'))
